...
<div class="max-w-2xl space-y-6">
  ...
  <!-- Voice Settings -->
  <div class="bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 p-4">
    <h3 class="font-medium mb-4">🎙️ Voice Interface</h3>
    <p class="text-sm text-slate-500 dark:text-slate-400 mb-4">
      Configure the voice conversation interface. Voice model must be downloaded separately.
      <a href="/voice" class="text-blue-400 underline ml-1">Open Voice →</a>
    </p>

    <div class="space-y-4">
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="block text-sm font-medium mb-2" for="v-agent-id">Default Agent ID</label>
          <input
            id="v-agent-id"
            type="text"
          ...
          />
          <p class="text-xs text-slate-500 mt-1">OpenClaw agent ID to talk to</p>
        </div>

        <div>
          <label class="block text-sm font-medium mb-2" for="v-stt-model">Whisper STT Model</label>
          <select
            id="v-stt-model"
            on:change={(e) => {
              if (typeof localStorage !== 'undefined') {
                localStorage.setItem('voice_stt_model', (e.target as HTMLSelectElement).value);
              }
            }}
            class="bg-gray-100 dark:bg-slate-700 border border-gray-300 dark:border-slate-600 rounded px-3 py-2 text-sm w-full"
          >
          ...
          <p class="text-xs text-slate-500 mt-1">Larger = better accuracy, slower start</p>
        </div>
      </div>

      <div>
        <label class="block text-sm font-medium mb-2">Voice Model Path (Piper TTS)</label>
        <div class="flex items-center gap-2">
          <input
            type="text"
            value={typeof localStorage !== 'undefined'
              ? localStorage.getItem('voice_model_path') || '~/.openclaw/piper-voices/en_US-amy-medium.onnx'
              : '~/.openclaw/piper-voices/en_US-amy-medium.onnx'}
            on:change={(e) => {
              if (typeof localStorage !== 'undefined') {
                localStorage.setItem('voice_model_path', (e.target as HTMLInputElement).value);
              }
            }}
            class="bg-gray-100 dark:bg-slate-700 border border-gray-300 dark:border-slate-600 rounded px-3 py-2 text-sm flex-1"
            placeholder="~/.openclaw/piper-voices/en_US-amy-medium.onnx"
          />
        </div>
        <p class="text-xs text-slate-500 mt-1">
          Download voices from
          <a href="https://huggingface.co/rhasspy/piper-voices" target="_blank" rel="noopener" class="text-blue-400 underline">
      ...
      <div class="flex items-center justify-between p-3 bg-slate-700/30 rounded-lg">
        <div>
          <div class="text-sm font-medium">Voice installed</div>
          <div class="text-xs text-slate-400">en_US-amy-medium at ~/.openclaw/piper-voices/</div>
        </div>
